            ...
            if (id == 254) {
                ...
                if (Packets.get(id)) {
                    ...
                    if (compressed) {
                        ...
                        lz4.decompressBlock(buf._getBuffer(buf.position()), this.#temp._getBuffer(), 0, size, 0);
    ...
    write(buf, object) {
        if (Buffer.isBuffer(object) || (object instanceof DataStream)) {
            ...
            if (length < 36 || object instanceof Packets.StreamChunk) {
                ...
                buf.put(1);
                let size = lz4.compressBlock(this.#temp._getBuffer(), buf._getBuffer(buf.position()), this.#temp.position(), this.#temp.limit(), lz4.makeBuffer(1 << 16));
                buf.position(buf.position() + size)
            }
        } else {
