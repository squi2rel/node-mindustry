...
class NetServer extends EventEmitter {
    ...
    constructor(game) {
        super();
        this.game = game;
        this.server = new Server(8192, new PacketSerializer(), (cid, p) => this.handleServerReceived(cid, p), () => this.handleMRequest(), this)
        this.limit = 30
        this.players = {}
        this.entities = {}
    }
    ...
    configDef(){
        this.config.syncTime = this.config.syncTime || 100;
    }
    setupIntervals(){
        this.syncInterval = setInterval(this.sync.bind(this), this.config.syncTime)
    }
    sync(){
        let entitysnap = this.entitySnapshot()
        this.sendBroadcast(entitysnap, false)
    }
    entitySnapshot(){
        let p = new Packets.EntitySnapshotCallPacket();
        let len = Object.keys(this.units).length
        p.amount = len;
        let buf = DataStream.allocate(2 ** 15);
        for(let i = 0; i < len; i++){
            let unit = Object.values(this.units)[i]
            buf.putInt(unit.id)
            buf.put(unit.type)
            UnitIO.write(buf, unit, false);
        }
        p.data = buf._getBuffer().slice(0, buf.position())
        return p
    }
    ...
    setMap(path){
        ...
        let m = this.game.world.map
        this.state.map = m.get("name")
    }
    getConnectedCount() {
