...
class BlockIO {
	static readMain(buf, type, ver){
		if(type == "GenericCrafter" || type == "Separator" || type == "HeatProducer" || type == "HeatCrafter"){
			...
			return result
		} else if(type.startsWith("Build")){
			let progress = buf.getFloat()
			let pid = buf.getShort()
			let rid = buf.getShort()
			let acsize = buf.get()
			let acc = []
			let totalacc = []
			let itemsLeft = []
			if(acsize != 255){
				for(let i = 0; i < acsize; i++){
					acc[i] = buf.getFloat()
					totalacc[i] = buf.getFloat()
					if(ver >= 1){
						itemsLeft[i] = buf.getInt()
					}
				}
			}
			let result = {
	...
	static writeMain(buf, type, ver, data) {
	    if (type == "GenericCrafter" || type == "Separator" || type == "HeatProducer" || type == "HeatCrafter") {
		    ...
		    buf.putFloat(data.progress);
		    buf.putShort(data.pid);
		    buf.putShort(data.rid);
		    let acsize = data.acsize;
		    buf.put(acsize);
		    if (acsize != 255) {
		        for (let i = 0; i < acsize; i++) {
		            buf.putFloat(data.acc[i]);
		            buf.putFloat(data.totalacc[i]);
		            if (ver >= 1) {
		                buf.putInt(data.itemsLeft[i]);
		            }
		        }
		    }
		}
