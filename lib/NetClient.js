...
class NetClient extends EventEmitter {
    ...
    constructor(game) {
        ...
        this.#client.on("disconnect", () => {
            ...
            if(this.config.autoReconnect){
                this.connect(this.port, this.ip, this.config)
            }
            this.reset();
    ...
    join(name, uuid, usid) {
        ...
        p.lang = this.config.lang ? this.config.lang : "en";
        p.client = this.config.client ? this.config.client : "official";
        p.mods = this.config.mods ? this.config.mods : []
        this.send(p, true)
    }
    sendChatMessage(msg) {
    ...
    loadWorld(packet) {
        let buf = DataStream.from(Buffer.from(pako.inflate(packet.stream)));
